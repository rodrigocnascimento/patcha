# AGENTS.md - Patcha CLI Development Guide

This file provides guidelines and commands for AI agents working on this codebase.

## Project Overview

Patcha is a CLI tool for scanning and patching vulnerabilities in Node.js dependencies. The source code is in `src/` and tests are in `tests/`.
...
- Prefer `interface` for public APIs and types
- Use `type` for unions, primitives, and internal types
- Always use `.js` extension for imports (required for ESM)

### Naming Conventions
- **Files**: kebab-case (e.g., `npm-audit.ts`, `scan-result.ts`)
- **Classes**: PascalCase (e.g., `PatchaError`, `ProjectNotFoundError`)
- **Functions**: camelCase (e.g., `buildDepTree`, `queryNpmAudit`)
- **Constants**: UPPER_SNAKE_CASE for configuration values
- **Interfaces/Types**: PascalCase

### Imports
Order imports as:
1. Built-in Node.js modules (path, fs, etc.)
...
### Error Handling
- Create custom error classes extending `PatchaError` with error codes
- Use descriptive error messages with context
- Handle errors at the CLI entry point with `handleError()`
...
- Export each command as a named export
- Use `.argument()` for positional arguments
- Use `.option()` for flags
- Use async action handlers
...
### Type Definitions
- Place shared types in `src/scanner/types.ts`
...
│   ├── arborist.ts  # @npmcli/arborist integration
│   ├── npm-audit.ts # npm audit API
...
- Use `dotenv/config` for loading .env files
- Provide sensible defaults with environment variable overrides
- Example: `process.env.NPM_AUDIT_API || 'https://registry.npmjs.org/-/npm/v1/security/audits'`
